...
public:
    ...
    {
        // rubies: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // rubies: get transaction coin age
...
public:
    ...
    // rubies: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // rubies: two types of block: proof-of-work or proof-of-stake
    ...
    // rubies: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // rubies: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // rubies: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // rubies: block index flags
    enum  
    {
